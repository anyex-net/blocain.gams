<template>
  <div class="app-container">
    ...
    <el-table v-loading="quoteStockLoading" :data="quoteStockData" style="width:100%;margin-bottom:20px;" border row-key="id">
      ...
      <el-table-column prop="marketCap" label="总市值" />
      <el-table-column prop="peRatio" label="市盈率">
        <template slot-scope="scope">
          <span v-if="scope.row.peRatio">{{ scope.row.peRatio + '%' }}</span>
        </template>
      </el-table-column>
      <el-table-column prop="pbRatio" label="市净率">
        <template slot-scope="scope">
          <span v-if="scope.row.pbRatio">{{ scope.row.pbRatio + '%' }}</span>
        </template>
      </el-table-column>
    ...
    <el-dialog title="股票行情" :visible.sync="quoteStockDialog" width="660px">
      <el-form ref="quoteStockForm" :inline="true" :model="quoteStockForm" :rules="quoteStockRules" label-width="100px" class="quoteStockForm">

        <el-form-item label="证券信息" prop="stockinfoId">
          <el-select v-model="quoteStockForm.stockinfoId" placeholder="请选择证券信息" value="">
        ...
        <el-form-item label="交易场所" prop="exchangeId">
          <el-select v-model="quoteStockForm.exchangeId" placeholder="请选择上市场所" value="">
        ...
        <el-form-item label="行情日期" prop="quoteDate">
          <el-date-picker v-model="quoteStockForm.quoteDate" type="date" value-format="timestamp" placeholder="选择行情日期" style="width:100%;"></el-date-picker>
        </el-form-item>
        <el-form-item label="昨收盘价" prop="prevClose">
          <el-input v-model="quoteStockForm.prevClose" />
        </el-form-item>
        <el-form-item label="最新价" prop="last">
          <el-input v-model="quoteStockForm.last" />
        </el-form-item>
        <el-form-item label="开盘价" prop="open">
          <el-input v-model="quoteStockForm.open" />
        </el-form-item>
        <el-form-item label="最高价" prop="high">
          <el-input v-model="quoteStockForm.high" />
        </el-form-item>
        <el-form-item label="最低价" prop="low">
          <el-input v-model="quoteStockForm.low" />
        </el-form-item>
        <el-form-item label="涨跌幅" prop="chg">
          <el-input v-model="quoteStockForm.chg">
            <span slot="suffix">%</span>
          </el-input>
        </el-form-item>
        <el-form-item label="涨跌额" prop="change">
          <el-input v-model="quoteStockForm.change" />
        </el-form-item>
        <el-form-item label="成交量" prop="volume">
          <el-input v-model="quoteStockForm.volume" />
        </el-form-item>
        <el-form-item label="成交额" prop="turnover">
          <el-input v-model="quoteStockForm.turnover" />
        </el-form-item>
        <el-form-item label="委比" prop="committeeThan">
          <el-input v-model="quoteStockForm.committeeThan" />
        </el-form-item>
        <el-form-item label="量比" prop="quantityRelative">
          <el-input v-model="quoteStockForm.quantityRelative" />
        </el-form-item>
        <el-form-item label="换手率" prop="turnoverRate">
          <el-input v-model="quoteStockForm.turnoverRate">
            <span slot="suffix">%</span>
          </el-input>
        </el-form-item>
        <el-form-item label="总市值" prop="marketCap">
          <el-input v-model="quoteStockForm.marketCap" />
        </el-form-item>
        <el-form-item label="市盈率" prop="peRatio">
          <el-input v-model="quoteStockForm.peRatio">
            <span slot="suffix">%</span>
          </el-input>
        </el-form-item>
        <el-form-item label="市净率" prop="pbRatio">
          <el-input v-model="quoteStockForm.pbRatio">
            <span slot="suffix">%</span>
          </el-input>
        </el-form-item>
        <el-form-item label="买一价" prop="bid">
          <el-input v-model="quoteStockForm.bid" />
        </el-form-item>
        <el-form-item label="卖一价" prop="ask">
          <el-input v-model="quoteStockForm.ask" />
        </el-form-item>
        <el-form-item label="买入量" prop="bidSize">
          <el-input v-model="quoteStockForm.bidSize" />
        </el-form-item>
        <el-form-item label="卖出量" prop="askSize">
          <el-input v-model="quoteStockForm.askSize" />
        </el-form-item>
        <el-divider></el-divider>
        <el-form-item>
          <el-button type="primary" @click="doSubmit('quoteStockForm')">提交</el-button>
        </el-form-item>
...
<script>
import { isBtnZeroToThousand, checkNum } from '../../utils/validate.js';
export default {
  name: 'QuoteStock',
  data() {
    return {
      quoteStockLoading: '',
      quoteStockDialog: false,
      quoteStockData: [],
      stockInfoData: [],
      exchangeData: [],
      quoteStockForm: {
      ...
      quoteStockRules: {
        stockinfoId: [
          { required: true, message: '请输入证券信息', trigger: 'change' }
        ],
        exchangeId: [
        ...
        volume: [
          { required: false, message: '请选择成交量', trigger: 'blur' }, { validator: checkNum, trigger: 'blur' }
        ],
        turnover: [
        ...
        marketCap: [
          { required: false, message: '请输入总市值', trigger: 'blur' }, { validator: checkNum, trigger: 'blur' }
        ],
        peRatio: [
          { required: false, message: '请输入市盈率', trigger: 'blur' }, { validator: checkNum, trigger: 'blur' }
        ],
        pbRatio: [
          { required: false, message: '请输入市净率', trigger: 'blur' }, { validator: checkNum, trigger: 'blur' }
        ],
        bid: [
  ...
  methods: {
    ...
    dialogAdd: function() {
      ...
      if (this.$refs.quoteStockForm) {
        this.$refs.quoteStockForm.resetFields();
      }
      this.quoteStockDialog = true;
    },
      ...
      this.$http({
        ...
        if (res.code === 200) {
          this.quoteStockForm = {
            ...
            'marketCap': res.object.marketCap,
            'peRatio': res.object.peRatio,
            'pbRatio': res.object.pbRatio,
            'bid': res.object.bid,
            'ask': res.object.ask,
    ...
    doSearch: function(page) {
      ...
      this.quoteStockLoading = true;
      this.$http({
        url: '/quote/stock/data',
        method: 'post',
        params: Object.assign(this.pageParams, this.searchForm)
      }).then(res => {
        if (res.code === 200) {
          this.quoteStockData = res.rows;
          this.pageParams.totalPage = res.totalPage;
          this.quoteStockLoading = false;
        }
      }).catch(error => {
        this.$message.error(error);
      });
    ...
    doSubmit: function(formName) {
      this.$refs[formName].validate((valid) => {
        if (valid) {
          this.$http({
            url: '/quote/stock/save',
            method: 'post',
            params: this.quoteStockForm
          }).then(res => {
            if (res.code === 200) {
          ...
          this.quoteStockDialog = false;
        }
      });
    },
    doDelete: function(row, column) {
      this.$confirm('确认删除该股票行情, 是否继续?', '提示', {
...
<style lang="scss">
  .quoteStockForm.el-form--inline .el-form-item__content {
    width: 196px;
  }
  .app-container {
    .el-input__suffix {
