<template>
  <div class="app-container">
    ...
    <el-table v-loading="quoteForeignExLoading" :data="quoteForeignExData" style="width:100%;margin-bottom:20px;" border row-key="id">
      ...
      <el-table-column prop="high52week" label="52周最高价" />
      <el-table-column prop="low52week" label="52周最低价" />
    ...
    <el-dialog title="外汇 行情" :visible.sync="quoteForeignExDialog" width="660px">
      <el-form ref="quoteForeignExForm" :inline="true" :model="quoteForeignExForm" :rules="quoteForeignExRules" label-width="100px" class="quoteForeignExForm">
        <el-form-item label="证券信息" prop="stockinfoId">
          <el-select v-model="quoteForeignExForm.stockinfoId" placeholder="请选择证券信息" value="">
        ...
        <el-form-item label="交易场所" prop="exchangeId">
          <el-select v-model="quoteForeignExForm.exchangeId" placeholder="请选择上市场所" value="">
        ...
        <el-form-item label="行情日期" prop="quoteDate">
          <el-date-picker v-model="quoteForeignExForm.quoteDate" type="date" value-format="timestamp" placeholder="选择行情日期" style="width:100%;"></el-date-picker>
        </el-form-item>
        <el-form-item label="昨收盘价" prop="prevClose">
          <el-input v-model="quoteForeignExForm.prevClose" />
        </el-form-item>
        <el-form-item label="最新价" prop="last">
          <el-input v-model="quoteForeignExForm.last" />
        </el-form-item>
        <el-form-item label="开盘价" prop="open">
          <el-input v-model="quoteForeignExForm.open" />
        </el-form-item>
        <el-form-item label="最高价" prop="high">
          <el-input v-model="quoteForeignExForm.high" />
        </el-form-item>
        <el-form-item label="最低价" prop="low">
          <el-input v-model="quoteForeignExForm.low" />
        </el-form-item>
        <el-form-item label="涨跌幅" prop="chg">
          <el-input v-model="quoteForeignExForm.chg">
            <span slot="suffix">%</span>
          </el-input>
        </el-form-item>
        <el-form-item label="涨跌额" prop="change">
          <el-input v-model="quoteForeignExForm.change" />
        </el-form-item>
        <el-form-item label="成交量" prop="volume">
          <el-input v-model="quoteForeignExForm.volume" />
        </el-form-item>
        <el-form-item label="成交额" prop="turnover">
          <el-input v-model="quoteForeignExForm.turnover" />
        </el-form-item>
        <el-form-item label="委比" prop="committeeThan">
          <el-input v-model="quoteForeignExForm.committeeThan" />
        </el-form-item>
        <el-form-item label="量比" prop="turnoverRate">
          <el-input v-model="quoteForeignExForm.turnoverRate" />
        </el-form-item>
        <el-form-item label="换手率" prop="quantityRelative">
          <el-input v-model="quoteForeignExForm.quantityRelative">
            <span slot="suffix">%</span>
          </el-input>
        </el-form-item>
        <el-form-item label="买一价" prop="bid">
          <el-input v-model="quoteForeignExForm.bid" />
        </el-form-item>
        <el-form-item label="卖一价" prop="ask">
          <el-input v-model="quoteForeignExForm.ask" />
        </el-form-item>
        <el-form-item label="买入量" prop="bidSize">
          <el-input v-model="quoteForeignExForm.bidSize" />
        </el-form-item>
        <el-form-item label="卖出量" prop="askSize">
          <el-input v-model="quoteForeignExForm.askSize" />
        </el-form-item>
        <el-form-item label="52周最高价" prop="high52week">
          <el-input v-model="quoteForeignExForm.high52week" />
        </el-form-item>
        <el-form-item label="52周最低价" prop="low52week">
          <el-input v-model="quoteForeignExForm.low52week" />
        </el-form-item>
        <el-divider></el-divider>
        <el-form-item>
          <el-button type="primary" @click="doSubmit('quoteForeignExForm')">提交</el-button>
        </el-form-item>
      </el-form>
    </el-dialog>
...
export default {
  ...
  data() {
    return {
      quoteForeignExLoading: '',
      quoteForeignExDialog: false,
      quoteForeignExData: [],
      stockInfoData: [],
      exchangeData: [],
      quoteForeignExForm: {
      ...
      quoteForeignExRules: {
        ...
        high52week: [
          { required: false, message: '请输入52周最高价', trigger: 'blur' }, { validator: isBtnZeroToThousand, trigger: 'blur' }, { validator: checkNum, trigger: 'blur' }
        ],
        low52week: [
          { required: false, message: '请输入52周最低价', trigger: 'blur' }, { validator: isBtnZeroToThousand, trigger: 'blur' }, { validator: checkNum, trigger: 'blur' }
        ],
        bid: [
  ...
  methods: {
    ...
    dialogAdd: function() {
      ...
      if (this.$refs.quoteForeignExForm) {
        this.$refs.quoteForeignExForm.resetFields();
      }
      this.quoteForeignExDialog = true;
    },
      ...
      this.$http({
        ...
        if (res.code === 200) {
          this.quoteForeignExForm = {
            ...
            'high52week': res.object.high52week,
            'low52week': res.object.low52week,
            'bid': res.object.bid,
            'ask': res.object.ask,
    ...
    doSearch: function(page) {
      ...
      this.quoteForeignExLoading = true;
      this.$http({
        url: '/quote/foreignex/data',
        method: 'post',
        params: Object.assign(this.pageParams, this.searchForm)
      }).then(res => {
        if (res.code === 200) {
          this.quoteForeignExData = res.rows;
          this.pageParams.totalPage = res.totalPage;
          this.quoteForeignExLoading = false;
        }
      }).catch(error => {
        this.$message.error(error);
      });
    ...
    doSubmit: function(formName) {
      this.$refs[formName].validate((valid) => {
        if (valid) {
          this.$http({
            url: '/quote/foreignex/save',
            method: 'post',
            params: this.quoteForeignExForm
          }).then(res => {
            if (res.code === 200) {
          ...
          this.quoteForeignExDialog = false;
        }
      });
    },
...
<style lang="scss">
  .quoteForeignExForm.el-form--inline .el-form-item__content {
    width: 196px;
  }
</style>
