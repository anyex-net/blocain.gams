<template>
  <div class="app-container">
    ...
    <el-table v-loading="quoteFuturesLoading" :data="quoteFuturesData" style="width:100%;margin-bottom:20px;" border row-key="id">
      ...
      <el-table-column prop="buyVolumeOutside" label="买盘" />
      <el-table-column prop="sellVolumeOutside" label="卖盘" />
    ...
    <el-dialog title="期货行情" :visible.sync="quoteFuturesDialog" width="660px">
      <el-form ref="quoteFuturesForm" :inline="true" :model="quoteFuturesForm" :rules="quoteFuturesRules" label-width="100px" class="quoteFuturesForm">
        <el-form-item label="证券信息" prop="stockinfoId">
          <el-select v-model="quoteFuturesForm.stockinfoId" placeholder="请选择证券信息" value="">
        ...
        <el-form-item label="交易场所" prop="exchangeId">
          <el-select v-model="quoteFuturesForm.exchangeId" placeholder="请选择上市场所" value="">
        ...
        <el-form-item label="行情日期" prop="quoteDate">
          <el-date-picker v-model="quoteFuturesForm.quoteDate" type="date" value-format="timestamp" placeholder="选择行情日期" style="width:100%;"></el-date-picker>
        </el-form-item>
        <el-form-item label="昨收盘价" prop="prevClose">
          <el-input v-model="quoteFuturesForm.prevClose" />
        </el-form-item>
        <el-form-item label="最新价" prop="last">
          <el-input v-model="quoteFuturesForm.last" />
        </el-form-item>
        <el-form-item label="开盘价" prop="open">
          <el-input v-model="quoteFuturesForm.open" />
        </el-form-item>
        <el-form-item label="最高价" prop="high">
          <el-input v-model="quoteFuturesForm.high" />
        </el-form-item>
        <el-form-item label="最低价" prop="low">
          <el-input v-model="quoteFuturesForm.low" />
        </el-form-item>
        <el-form-item label="涨跌幅" prop="chg">
          <el-input v-model="quoteFuturesForm.chg">
            <span slot="suffix">%</span>
          </el-input>
        </el-form-item>
        <el-form-item label="涨跌额" prop="change">
          <el-input v-model="quoteFuturesForm.change" />
        </el-form-item>
        <el-form-item label="成交量" prop="volume">
          <el-input v-model="quoteFuturesForm.volume" />
        </el-form-item>
        <el-form-item label="成交额" prop="turnover">
          <el-input v-model="quoteFuturesForm.turnover" />
        </el-form-item>
        <el-form-item label="委比" prop="committeeThan">
          <el-input v-model="quoteFuturesForm.committeeThan" />
        </el-form-item>
        <el-form-item label="量比" prop="quantityRelative">
          <el-input v-model="quoteFuturesForm.quantityRelative" />
        </el-form-item>
        <el-form-item label="换手率" prop="turnoverRate">
          <el-input v-model="quoteFuturesForm.turnoverRate">
            <span slot="suffix">%</span>
          </el-input>
        </el-form-item>
        <el-form-item label="买盘(外盘)" prop="buyVolumeOutside">
          <el-input v-model="quoteFuturesForm.buyVolumeOutside" />
        </el-form-item>
        <el-form-item label="卖盘(内盘)" prop="sellVolumeOutside">
          <el-input v-model="quoteFuturesForm.sellVolumeOutside" />
        </el-form-item>
        <el-form-item label="持仓量" prop="positionVolume">
          <el-input v-model="quoteFuturesForm.positionVolume" />
        </el-form-item>
        <el-form-item label="买一价" prop="bid">
          <el-input v-model="quoteFuturesForm.bid" />
        </el-form-item>
        <el-form-item label="卖一价" prop="ask">
          <el-input v-model="quoteFuturesForm.ask" />
        </el-form-item>
        <el-form-item label="买入量" prop="bidSize">
          <el-input v-model="quoteFuturesForm.bidSize" />
        </el-form-item>
        <el-form-item label="卖出量" prop="askSize">
          <el-input v-model="quoteFuturesForm.askSize" />
        </el-form-item>
        <el-divider></el-divider>
        <el-form-item>
          <el-button type="primary" @click="doSubmit('quoteFuturesForm')">提交</el-button>
        </el-form-item>
      </el-form>
    </el-dialog>
...
export default {
  ...
  methods: {
    ...
    dialogAdd: function() {
      ...
      if (this.$refs.quoteFuturesForm) {
        this.$refs.quoteFuturesForm.resetFields();
      }
      this.quoteFuturesDialog = true;
    },
    ...
    doSearch: function(page) {
      ...
      this.quoteFuturesLoading = true;
      this.$http({
        url: '/quote/futures/data',
        method: 'post',
        params: Object.assign(this.pageParams, this.searchForm)
      }).then(res => {
        if (res.code === 200) {
          this.quoteFuturesData = res.rows;
          this.pageParams.totalPage = res.totalPage;
          this.quoteFuturesLoading = false;
        }
      }).catch(error => {
        this.$message.error(error);
      });
    ...
    doSubmit: function(formName) {
      this.$refs[formName].validate((valid) => {
        if (valid) {
          this.$http({
            url: '/quote/futures/save',
            method: 'post',
            params: this.quoteFuturesForm
          }).then(res => {
            if (res.code === 200) {
...
<style lang="scss">
  .quoteFuturesForm.el-form--inline .el-form-item__content {
    width: 196px;
  }
</style>
