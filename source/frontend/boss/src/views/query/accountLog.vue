  ...
  <div class="app-container">
    <el-form ref="searchForm" :model="searchForm" :inline="true" size="mini" :rules="searchRules">
      <el-form-item label="用户ID" prop="accountId">
        <el-input v-model="searchForm.accountId" clearable placeholder="请输入用户ID"></el-input>
      </el-form-item>
      <el-form-item label="用户名称">
        <el-input v-model="searchForm.accountName" clearable placeholder="请输入用户名称"></el-input>
      </el-form-item>
    ...
    <el-table v-loading="accountLogLoading" :data="accountLogData" style="width:100%;margin-bottom:20px;" border row-key="id">
      <el-table-column prop="accountId" label="用户ID" />
      <el-table-column prop="accountName" label="用户名称" />
  ...
  methods: {
    ...
    doSearch: function(page) {
      ...
      this.$refs.searchForm.validate((valid) => {
        if (valid) {
          this.accountLogLoading = true;
          this.$http({
            url: '/query/accountLog/data',
            method: 'post',
            params: Object.assign(this.pageParams, this.searchForm)
          }).then(res => {
            if (res.code === 200) {
              this.accountLogData = res.rows;
              this.pageParams.totalPage = res.totalPage;
              this.accountLogLoading = false;
            }
          }).catch(error => {
...
<style lang="scss">
  .accountLogForm.el-form--inline .el-form-item__content {
    width: 196px;
  }
</style>
