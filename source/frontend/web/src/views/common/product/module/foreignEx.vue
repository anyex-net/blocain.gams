<template>
  <div class="app-container">
    <div class="product-wrapper">
      ...
      <el-table :data="foreignData" :empty-text="$t('common.product.no_data')" class="product-table"> <!--无数据-->
        <el-table-column prop="stockCode" :label="$t('common.product.forEx.code')" /><!--外汇代码-->
        ...
        <el-table-column prop="prevClose" :label="$t('common.product.close_price')" align="right"><!--昨收盘价-->
          <template slot-scope="scope">
            <span>{{ scope.row.prevClose | keepNumFour }}</span>
          </template>
        </el-table-column>
        <el-table-column prop="last" :label="$t('common.product.last_price')" align="right"><!--最新价-->
          <template slot-scope="scope">
            <span>{{ scope.row.last | keepNumFour }}</span>
          </template>
        </el-table-column>
...
export default {
  ...
  methods: {
    ...
    doInitData(page) {
      ...
      this.$http({
        url: '/stock/quote/foreignex',
        method: 'get',
        params: this.pageParams
      }).then(res => {
        if (res.code === 200) {
          this.foreignData = res.rows;
