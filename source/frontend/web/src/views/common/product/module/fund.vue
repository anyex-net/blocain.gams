<template>
  <div class="app-container">
    <div class="product-wrapper">
      ...
      <el-table :data="fundData" :empty-text="$t('common.product.no_data')" class="product-table"> <!--无数据-->
        <el-table-column prop="stockCode" :label="$t('common.product.funds.code')" /><!--基金代码-->
...
export default {
  ...
  methods: {
    ...
    doInitData(page) {
      ...
      this.$http({
        url: '/stock/quote/fund',
        method: 'get',
        params: this.pageParams
      }).then(res => {
        if (res.code === 200) {
          this.fundData = res.rows;
