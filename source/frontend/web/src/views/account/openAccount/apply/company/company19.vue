<template>
  <div>
    <div id="ZC-regiMain">
      <div class="topBG">
        <div class="titleIMG">{{ '-' + $t('registerOpenAccount.header.openAccount') + '(17/17) -' }}</div>
        ...
        <div class="contentBOXdignwei">
          ...
          >
            <div class="formContent">
              <div v-if="company19.isAllUpload !== 1" class="formNape" style="margin-bottom: 10px;">
                    ...
                    <h3>
                      <ul class="statusTitle">
                        <li>
                          ...
                          <span v-if="company19.isAllUpload !== 0 || company19.isAllUpload !== 1">{{ $t('registerOpenAccount.openAccount.common.filesUpload.status.prove') }}</span> <!-- $i18n - 未完成，等待其它证明文件 -->
                          <span v-if="company19.isAllUpload === 0">{{ $t('registerOpenAccount.openAccount.common.filesUpload.status.status') }}</span> <!-- $i18n - 完成开户,待审核 -->
                        </li>
              ...
              <div v-if="company19.isAllUpload !== 1" class="form-content">
                <!-- 电子上传 -->
                <div class="agreement1">
                  <div
                    class="agreement-top xiaoshou"
                    @click="company19.electronicMessage.isUnfold=!company19.electronicMessage.isUnfold"
                  >
                    ...
                    <div class="left20 zhuce2ji right1 right20 JYxialaanniu" :class="{ JYxialaanniuhover: company19.electronicMessage.isUnfold}" />
                  </div>
                  <div v-show="company19.electronicMessage.isUnfold" class="fileStateTable">
                    <div class="top20">
                    ...
                    <div class="gudongTable">
                      <table border="1" cellspacing="0" cellpadding="0">
                        ...
                        <tbody :class="{ tbodyEN: lang === 'en_US' }">
                          <tr v-for="(item,index) in company19.electronicMessage.tableList" :key="index">
                            <td class="textAlignL " style="text-align: left; padding-left:20px;line-height: 21px;">{{ item.content }}</td>
                            <td>
                              <span>{{ item.stateTextClient }}</span>
                              <span v-if="company19.isAllUpload !== 0 && company19.isAllUpload !== 1" class="shagnchuanbtn" @click="UploadBtn(index)">{{ $t('registerOpenAccount.common.btn.upload') }}</span> <!-- $i18n - 上传 -->
                            ...
                            <td class="CORred xiaoshou" @click="getExaminefile(item.moreInfoPdf)">{{ $t('registerOpenAccount.openAccount.common.filesUpload.common.more') }}</td> <!-- $i18n - 更多信息 -->
                          </tr>
                        </tbody>
                      </table>
                    </div>
                ...
                <div class="agreement1 top20">
                  <div
                    class="agreement-top xiaoshou"
                    @click="company19.emailMessage.isUnfold=!company19.emailMessage.isUnfold"
                  >
                    ...
                    <div class="left20 zhuce2ji right1 right20 JYxialaanniu" :class="{ JYxialaanniuhover: company19.emailMessage.isUnfold}" />
                  </div>
                  <div v-show="company19.emailMessage.isUnfold" class="fileStateTable">
                    <div class="top20">
                      <p class="zhuce3ji">{{ $t('registerOpenAccount.openAccount.common.filesUpload.emailFile.describe1a') }} {{ company19.emailMessage.email }}</p> <!-- $i18n - 请将您的电子文件通过电子邮件发送到 -->
                    ...
                    <div class="gudongTable">
                      <table border="1" cellspacing="0" cellpadding="0">
                        ...
                        <tbody>
                          <tr v-for="(item,index) in company19.emailMessage.tableList" :key="index">
                            <td class="textAlignL" style="text-align: left; padding-left:20px">{{ item.content }}</td>
                            <td class="CORred xiaoshou" @click="getExaminefile(item.moreInfoPdf)">{{ $t('registerOpenAccount.openAccount.common.filesUpload.common.more') }}</td> <!-- $i18n - 更多信息 -->
                          </tr>
                        </tbody>
                      </table>
              ...
              <div v-if="company19.isfileModal" class="Modalframe">
                <div class="MFcontent">
                  <div class="MFform">
                    <el-upload
                      ref="upload"
...
export default {
  data: function() {
    return {
      ...
      company19: {
        isAllUpload: '', // 是否完成开户状态 0待审核 1已审核 其他未审核
        ...
        url: '', // 传给后端
        fileName: '',
        userName: { // 机构名-被授权人名 ---> 完成展示姓名替换
          organizationName: '', // 机构名
          authorizedName: '' // 被授权人名
        },
  ...
  methods: {
    ...
    openAccount() {
      this.$store.dispatch('user/getInfo').then(res => {
        if (res.code === 200) {
          // 0 待审核 1 已审核 其他为未审核
          this.company19.isAllUpload = res.object.registerStatus;
          this.getUserName();
    ...
    getUserName() { // 获取 机构名-被授权人名 ---> 完成展示姓名替换
      this.getOrganizationName().then(res => {
        this.getAuthorizedName().then(res => {
          this.getTableList();
        });
      });
    },
          ...
          .then(res => {
            if (res.code === 200) {
              if (this.$verify.resStatus(res)) {
                this.company19.userName.organizationName = res.object.institutionalName;
                resolve(res);
              }
            } else if (res.code === 2003) {
              if (!this.isTokenHint) {
      ...
      return new Promise((resolve, reject) => {
        this.$http({
          ...
          .then(res => {
            if (res.code === 200) {
              if (this.$verify.resStatus(res)) {
                this.company19.userName.authorizedName = res.object[0].lastName + ' ' + res.object[0].middleName + ' ' + res.object[0].firstName + ' ' + res.object[0].suffix;
                resolve(res);
              }
            } else if (res.code === 2003) {
              if (!this.isTokenHint) {
    ...
    getTableList() { // 获取表格list
      this.$http({
        url: '/openAccount/company/getFileUploadType',
        method: 'get',
        params: { lang: this.lang, IEgetIssue: this.$verify.timestampRandom() }
      }).then(res => {
        if (res.code === 200) {
          if (this.$verify.resStatus(res)) {
            for (let i = 0; i < res.object.uploadFileTypeList.length; i++) {
              if (res.object.uploadFileTypeList[i].content.indexOf('company') !== -1) {
                res.object.uploadFileTypeList[i].content = res.object.uploadFileTypeList[i].content.replace(/company/g, this.company19.userName.organizationName);
              } else if (res.object.uploadFileTypeList[i].content.indexOf('authorizedPerson') !== -1) {
                res.object.uploadFileTypeList[i].content = res.object.uploadFileTypeList[i].content.replace(/authorizedPerson/g, this.company19.userName.authorizedName);
              }
              res.object.uploadFileTypeList[i].stateTextClient = this.$i18n.t('registerOpenAccount.openAccount.common.filesUpload.modal.hint3'); // $i18n - 未上传
              res.object.uploadFileTypeList[i].fileTypeID = '';
              res.object.uploadFileTypeList[i].ifExamineClient = false;
              this.company19.electronicMessage.tableList.push(JSON.parse(JSON.stringify(res.object.uploadFileTypeList[i])));
              this.gerFileIdentification(i);
            }
            this.company19.emailMessage.tableList = this.company19.electronicMessage.tableList;
          }
        } else if (res.code === 2003) {
          if (!this.isTokenHint) {
            this.isTokenHint = true;
    ...
    beforeAvatarUpload(file) { // // ---- 上传文件之前
      ...
      this.company19.fileSuffix = testmsg;
      return status;
    },
    beforeRemove(file, fileList) { // ---- 删除文件之前
      if (this.company19.isBeBeingUpload === true) {
    ...
    customUpload(param) { // http-request 自定义上传
      ...
      this.company19.isBeBeingUpload = true;
      this.generateUploadObjectPresignedUrl().then(res => {
        this.fileList[0].percentage = 30;
        this.company19.presignedUrl = res.object.presignedUrl;
        this.company19.url = res.object.url;
        this.UploadAWS(param);
      });
    },
    generateUploadObjectPresignedUrl() { // 获取上传文件加密后的路径
      this.company19.fileName = this.$store.state.user.applyData.xinxi.id + '-' +
                      this.company19.electronicMessage.tableList[this.company19.fileIndex].typeKey + '-' +
                      this.company19.electronicMessage.tableList[this.company19.fileIndex].type + '-' +
                      this.company19.electronicMessage.tableList[this.company19.fileIndex].coPersonalId + '.' +
                      this.company19.fileSuffix;
      return new Promise((resolve, reject) => {
        this.$http({
          url: 'openAccount/company/generateUploadObjectPresignedUrl',
          method: 'get',
          params: { dir: 'AccountFile', fileName: this.company19.fileName, IEgetIssue: this.$verify.timestampRandom() }
        })
          .then(res => {
            if (res.code === 200) {
              if (this.$verify.resStatus(res)) {
    ...
    UploadAWS(param) { // 文件上传到aws桶
      this.$http({
        url: this.company19.presignedUrl,
        method: 'put',
        data: param.file
      })
        .then(res => {
          ...
          this.company19.isBeBeingUpload = false;
          this.$message.error(error || this.$i18n.t('registerOpenAccount.openAccount.common.filesUpload.common.uploadDefeated')); // $i18n - 上传失败
          this.fileInitialize();
        });
    },
    UploadAfterEnd() { // 上传文件信息至后端
      ...
      const data = {
        ...
        createdBy,
        createdDate,
        fileName: this.company19.fileName,
        filePath: this.company19.url,
        coPersonalId: this.company19.electronicMessage.tableList[this.company19.fileIndex].coPersonalId,
        id: this.company19.electronicMessage.tableList[this.company19.fileIndex].fileTypeID,
        fileIdentification: this.company19.electronicMessage.tableList[this.company19.fileIndex].typeKey
      };
      this.$http({
        url: '/openAccount/company/saveNineteen',
        method: 'post',
        data
      }).then(res => {
        if (res.code === 200) {
          ...
          this.gerFileIdentification(this.company19.fileIndex);
        } else if (res.code === 2003) {
          if (!this.isTokenHint) {
            this.isTokenHint = true;
            this.$verify.staleDated(this);
    ...
    gerFileIdentification(index) { // 获取文件路径
      return new Promise((resolve, reject) => {
        this.$http({
          url: '/account/accountCoFile/findByAccountId/fileIdentification',
          method: 'get',
          params: { coPersonalId: this.company19.electronicMessage.tableList[index].coPersonalId, fileIdentification: this.company19.electronicMessage.tableList[index].typeKey, lang: this.lang, IEgetIssue: this.$verify.timestampRandom() }
        })
          .then(res => {
            if (res.code === 200) {
              if (this.$verify.resStatus(res)) {
                this.company19.electronicMessage.tableList[index].fileTypeID = res.object[0].id;
                this.company19.electronicMessage.tableList[index].filePathClient = res.object[0].filePath;
                this.company19.electronicMessage.tableList[index].stateTextClient = this.$i18n.t('registerOpenAccount.openAccount.common.filesUpload.common.uploadSuccessful'); // <!-- $i18n - 上传成功 -->
                this.company19.electronicMessage.tableList[index].ifExamineClient = true;
    ...
    filesUploadStatus() { // 文件状态,是否全部上传
      let status = 0;
      if (this.company19.electronicMessage.tableList.length !== 0) {
        for (let i = 0; i < this.company19.electronicMessage.tableList.length; i++) {
          if (this.company19.electronicMessage.tableList[i].ifExamineClient) {
            status++;
          }
        }
      }
      if (status === this.company19.electronicMessage.tableList.length && this.company19.isAllUpload !== 1) {
        this.$http({
          url: '/openAccount/company/openAccountFinish',
          method: 'post',
          params: { status: 1, lang: this.lang }
        })
          .then(res => {
            if (res.code === 200) {
              this.company19.isAllUpload = 0;
    ...
    UploadBtn(index) { // 上传btn点击
      this.company19.isfileModal = true;
      this.company19.fileIndex = index;
    },
    ...
    fileInitialize(isremoveModalBox = false) { // 文件列表初始化+进度条初始化
      this.fileList = [];
      this.company19.isfileModal = !isremoveModalBox;
    },
