            ...
            <div class="formContent">
                ...
                <div class="formNapeContent">
                  ...
                  <div>
                    <div class="form-content ifBtnStyle">
                      <!-- 1 -->
                      <div v-for="(item,index) in company18.questionnaireList" :key="index">
                        <div class="isBtn">
                          <ul ref="istext">
              ...
              <div class="formbtn">
                <div class="formbtn-content">
                  <el-form-item>
                    <el-button class="right-btn BTbtn" plain @click="submitForm('company18')">{{ $t('registerOpenAccount.footer.btnYes') }}</el-button>
...
export default {
  ...
  methods: {
    ...
    getAccountNameMessage() { // 获取用户已填入的信息
      this.$http({
        url: '/account/accountCoAmlCtf/findByAccountId',
        method: 'get',
        ...
        if (res.code === 200) {
          if (this.$verify.resStatus(res)) {
            ...
            for (let i = 0; i < res.object.length; i++) {
              this.company18.questionnaireList[i].ifbtn = res.object[i].status;
              this.company18.questionnaireList[i].fillInput = res.object[i].supplementaryInfo;
            }
          }
    ...
    issueList() { // 获取问卷列表
      this.$http({
        ...
        if (res.code === 200) {
          ...
          this.company18.questionnaireList = res.object;
        ...
        .then(res => {
          if (res.code === 200) {
            if (this.$verify.resStatus(res)) {
              ...
              this.company18.identityInformation.authorizedPerson = appellation + lastName + ' ' + middleName + ' ' + firstName + ' ' + suffix;
            }
          } else if (res.code === 2003) {
            if (!this.isTokenHint) {
              this.isTokenHint = true;
          ...
          if (res.code === 200) {
            if (this.$verify.resStatus(res)) {
              ...
              this.$verify.State1(this.$store.state.user.applyData.xinxi.country, _this, this.lang).then(res => {
                this.company18.identityInformation.location = res;
              });
        ...
        if (res.code === 200) {
          for (let i = 0; i < res.object.length; i++) {
            if (res.object[i].dictValue === idCardType) {
              ...
              this.company18.identityInformation.identificationPaper = issuingCountry + ' , ' + idCardType + ' , ' + idNumber;
            }
          }
        } else if (res.code === 2003) {
          if (!this.isTokenHint) {
    ...
    getSignature() { // 账户名称 - 第一页机构信息
      this.$http({
        ...
        .then(res => {
          if (res.code === 200) {
            if (this.$verify.resStatus(res)) {
              this.company18.identityInformation.accountName = res.object.institutionalName;
            }
          } else if (res.code === 2003) {
            if (!this.isTokenHint) {
              this.isTokenHint = true;
    ...
    submitFormData() {
      ...
      if (this.newValue === null) {
        for (let i = 0; i < this.company18.questionnaireList.length; i++) {
          data.push({
            accountId,
            createdBy,
            createdDate,
            issue: this.company18.questionnaireList[i].dictValue,
            status: this.company18.questionnaireList[i].ifbtn,
            supplementaryInfo: this.company18.questionnaireList[i].ifbtn ? this.company18.questionnaireList[i].fillInput : null
          });
        }
      } else {
        for (let i = 0; i < this.company18.questionnaireList.length; i++) {
          this.newValue[i].issue = this.company18.questionnaireList[i].dictValue;
          this.newValue[i].status = this.company18.questionnaireList[i].ifbtn;
          this.newValue[i].supplementaryInfo = this.company18.questionnaireList[i].ifbtn ? this.company18.questionnaireList[i].fillInput : null;
        }
        data = this.newValue;
      }
      this.$http({
        url: '/openAccount/company/saveEighteen',
        method: 'post',
        data
      })
        .then(res => {
          if (res.code === 200) {
            this.$verify.routerPermissions({ path: '/openAccount/company19' });
          } else if (res.code === 2003) {
            if (!this.isTokenHint) {
              this.isTokenHint = true;
              this.$verify.staleDated(this);
