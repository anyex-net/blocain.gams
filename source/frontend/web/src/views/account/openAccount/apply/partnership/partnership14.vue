            ...
            <div class="formContent">
              ...
              <div class="formNape">
                ...
                <div class="formNapeContent">
                  <div class="form-content">
                    ...
                    <!-- 1 -->
                    <div v-for="(item,index) in partnership14.agreement" :key="index">
                      <div class="agreement">
                    ...
                    <div class="signature" style="margin-top: 60px;">
                      <ul class="bottom10 clearfix">
                        ...
                        <li class="left1">{{ partnership14.shengming.signatureZhanshi }}</li>
                      </ul>
                      <el-form-item class="zhuce3ji">
                        <el-input v-model="partnership14.shengming.signature" maxlength="128" />
                      </el-form-item>
                      <div v-show="partnership14.qianming" class="CORred">{{ $t('registerOpenAccount.common.formPrompt.signatureHint') }}</div> <!-- $i18n - 签名与上方不一致 或 为空 -->
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div class="formbtn">
                <div class="formbtn-content">
                  <el-form-item>
                    <el-button class="right-btn BTbtn" plain @click="submitForm('partnership14')">{{ $t('registerOpenAccount.footer.btnYes') }}</el-button>
...
export default {
  ...
  watch: {
    partnership14: {// 深度监听，签名提示框显示隐藏
      handler(val, oldVal) {
        if (this.partnership14.shengming.signature === this.partnership14.shengming.signatureZhanshi) {
          this.partnership14.qianming = false;
        } else {
          this.partnership14.qianming = true;
        }
      },
      deep: true
    }
  ...
  methods: {
    ...
    // 获取用户已填入的信息
    // accountParAgreementFilling() {
    //   this.$http({
    //     url: '/account/accountParAgreement/findByAccountId',
    //     method: 'get',
    ...
    //             this.partnership14.agreement[i].agreementNumber = res.object[i].agreementNumber;
    //             this.partnership14.shengming.signature = res.object[i].signature;
    //             this.partnership14.shengming.riqi = this.$verify.NowDate3();
    //             this.partnership14.agreement[i].isBtnStyle = res.object[i].status;
    //           }
    //         }
    ...
    getAgreement() {
      this.$http({
        url: '/openAccount/partnerShip/getAgreement',
        method: 'get',
        ...
        .then(res => {
          if (res.code === 200) {
            for (let i = 0; i < res.object.length; i++) {
              ...
              this.partnership14.agreement.push(
                JSON.parse(JSON.stringify(res.object[i]))
              );
            }
            // this.accountParAgreementFilling();
          } else if (res.code === 2003) {
            if (!this.isTokenHint) {
    ...
    getSignature(personalType) { // 签名
      this.$http({
        ...
        .then(res => {
          if (res.code === 200) {
            if (this.$verify.resStatus(res)) {
              this.partnership14.shengming.zhanghuName = res.object[0].lastName + ' ' + `${res.object[0].middleName ? res.object[0].middleName + ' ' : res.object[0].middleName + ''}` + res.object[0].firstName;
              this.partnership14.shengming.riqi = this.$verify.NowDate3();
              this.partnership14.shengming.signatureZhanshi = res.object[0].lastName + ' ' + `${res.object[0].middleName ? res.object[0].middleName + ' ' : res.object[0].middleName + ''}` + res.object[0].firstName;
              this.partnership14.shengming.signature = res.object[0].lastName + ' ' + `${res.object[0].middleName ? res.object[0].middleName + ' ' : res.object[0].middleName + ''}` + res.object[0].firstName;
            }
          } else if (res.code === 2003) {
            if (!this.isTokenHint) {
              this.isTokenHint = true;
    ...
    // 同意true,false点击事件
    isBtnT(index) {
      this.partnership14.agreement[index].isconsent = false;
      this.partnership14.agreement[index].isBtnStyle = true;
    },
    isBtnF(index) {
      this.partnership14.agreement[index].isconsent = true;
      this.partnership14.agreement[index].isBtnStyle = false;
    },
    // 提交
    submitForm() {
      let index = 0;
      for (let i = 0; i < this.partnership14.agreement.length; i++) {
        if (this.partnership14.agreement[i].isBtnStyle === false) {
          this.partnership14.agreement[i].isconsent = true;
        } else {
          index++;
        }
      }
      if (index === this.partnership14.agreement.length) {
        if (this.partnership14.qianming === false) {
          this.submitFormData();
        }
      } else {
    ...
    submitFormData() {
      ...
      if (this.newValue === null) {
          ...
          data.push(
            {
              ...
              agreementNumber: this.partnership14.agreement[i].agreementNumber,
              signature: this.partnership14.shengming.signature,
              signatureTime: this.$verify.NowDate(this.partnership14.shengming.riqi),
              status: this.partnership14.agreement[i].isBtnStyle
            }
          );
        }
      } else {
        for (let i = 0; i < this.partnership14.agreement.length; i++) {
          ...
          this.newValue[i].agreementNumber = this.partnership14.agreement[i].agreementNumber;
          this.newValue[i].signature = this.partnership14.shengming.signature;
          this.newValue[i].signatureTime = this.$verify.NowDate(this.partnership14.shengming.riqi);
          this.newValue[i].status = this.partnership14.agreement[i].isBtnStyle;
        }
        data = this.newValue;
      }
      this.$http({
        url: '/openAccount/partnerShip/saveFourteen',
        method: 'post',
        data
      })
        .then(res => {
          if (res.code === 200) {
            this.$verify.routerPermissions({ path: '/openAccount/partnership15' });
          } else if (res.code === 2003) {
            if (!this.isTokenHint) {
              this.isTokenHint = true;
              this.$verify.staleDated(this);
    ...
    beforepage() {
      this.$http({ // 获取12页信息
        url: '/account/accountParCrsEntity/findByAccountId',
        method: 'get',
        params: { lang: this.lang, IEgetIssue: this.$verify.timestampRandom() }
      })
        .then(res => {
          if (res.code === 200) {
            if (this.$verify.resStatus(res)) {
              if (res.object.accountParCrsEntityContrList.length > 0 && res.object.accountParCrsEntity.entityNumber === 'Passive NFE') {
                this.accountParCrsEntityContrList();
              } else {
    ...
    accountParCrsEntityContrList() { // 存储id 13页获取第一条控权人
      this.$http({
        url: '/account/accountParCrsEntityContr/findByAccountId',
        method: 'get',
        params: { lang: this.lang, IEgetIssue: this.$verify.timestampRandom() }
      })
        .then(res => {
          if (res.code === 200) {
            if (this.$verify.resStatus(res)) {
              ...
              this.$verify.routerPermissions({ path: '/openAccount/partnership13' });
            }
          } else if (res.code === 2003) {
            if (!this.isTokenHint) {
              this.isTokenHint = true;
