...
class EDecoder implements ObjectInput
{
    ...
    {
        int reqId = readInt(), tickCount = readInt();
        ...
        for (int i = 0; i < tickCount; i++)
        {
            ...
            long size = readLong();
            String exchange = readStr(), specialConditions = readStr();
    ...
    private void processHistoricalTicksBidAsk() throws IOException
    {
        ...
        for (int i = 0; i < tickCount; i++)
        {
            ...
            double priceBid = readDouble(), priceAsk = readDouble();
            long sizeBid = readLong(), sizeAsk = readLong();
    ...
    private void processMarketDepthL2Msg() throws IOException
    {
        ...
        m_EWrapper.updateMktDepthL2(id, position, marketMaker, operation, side, price, size, isSmartDepth);
    }
    ...
    private void processExecutionDataMsg() throws IOException
    {
        ...
        exec.side(readStr());
        if (m_serverVersion >= EClient.MIN_SERVER_VER_FRACTIONAL_POSITIONS) exec.shares(readDouble());
        else exec.shares(readInt());
    ...
    private void processOrderStatusMsg() throws IOException
    {
        ...
        m_EWrapper.orderStatus(id, status, filled, remaining, avgFillPrice, permId, parentId, lastFillPrice, clientId, whyHeld, mktCapPrice);
    }
    ...
    private void processTickEFPMsg() throws IOException
    {
        ...
        m_EWrapper.tickEFP(tickerId, tickType, basisPoints, formattedBasisPoints, impliedFuturesPrice, holdDays, futureLastTradeDate, dividendImpact,
    ...
    private void processTickOptionComputationMsg() throws IOException
    {
        ...
        if (Double.compare(impliedVol, -1) == 0)
        { // -1 is the "not yet computed" indicator
            impliedVol = Double.MAX_VALUE;
        }
        double delta = readDouble();
        if (Double.compare(delta, -2) == 0)
        ...
        { // introduced in version == 5
            optPrice = readDouble();
            if (Double.compare(optPrice, -1) == 0)
            { // -1 is the "not yet computed" indicator
                optPrice = Double.MAX_VALUE;
            }
            pvDividend = readDouble();
            if (Double.compare(pvDividend, -1) == 0)
            { // -1 is the "not yet computed" indicator
        ...
        {
            gamma = readDouble();
            if (Double.compare(gamma, -2) == 0)
            { // -2 is the "not yet computed" indicator
                gamma = Double.MAX_VALUE;
            }
            vega = readDouble();
            if (Double.compare(vega, -2) == 0)
            { // -2 is the "not yet computed" indicator
                vega = Double.MAX_VALUE;
            }
            theta = readDouble();
            if (Double.compare(theta, -2) == 0)
            { // -2 is the "not yet computed" indicator
                theta = Double.MAX_VALUE;
            }
            undPrice = readDouble();
            if (Double.compare(undPrice, -1) == 0)
            { // -1 is the "not yet computed" indicator
