...
public abstract class EClient
{
    ...
    // all supported server versions are listed below
    ...
    public void disableUseV100Plus()
    {
        if (isConnected())
        {
            m_eWrapper.error(EClientErrors.NO_VALID_ID, EClientErrors.ALREADY_CONNECTED.code(), EClientErrors.ALREADY_CONNECTED.msg());
            return;
        }
        m_connectOptions = "";
        m_useV100Plus = false;
            ...
            List<TagValue> scannerSubscriptionFilterOptions)
    {
        ...
        {
            error(EClientErrors.NO_VALID_ID, EClientErrors.UPDATE_TWS, " It does not support API scanner subscription generic filter options");
            return;
        }
        final int VERSION = 4;
        try
    ...
    public synchronized void reqMktData(int tickerId, Contract contract, String genericTickList, boolean snapshot, boolean regulatorySnapshot,
            ...
            int useRTH, int formatDate, boolean keepUpToDate, List<TagValue> chartOptions)
    {
        ...
        try
        {
            ...
            if (m_serverVersion < MIN_SERVER_VER_TRADING_CLASS)
            {
                ...
                {
                    error(tickerId, EClientErrors.UPDATE_TWS, "  It does not support conId and tradingClass parameters in reqHistoricalData.");
                    return;
                }
            }
            Builder b = prepareBuffer();
    ...
    public synchronized void reqHeadTimestamp(int tickerId, Contract contract, String whatToShow, int useRTH, int formatDate)
    {
        ...
        try
        {
            ...
            {
                error(tickerId, EClientErrors.UPDATE_TWS, "  It does not support head time stamp requests.");
                return;
            }
            Builder b = prepareBuffer();
    ...
    public synchronized void cancelHeadTimestamp(int tickerId)
    {
        ...
        try
        {
            ...
            {
                error(tickerId, EClientErrors.UPDATE_TWS, "  It does not support head time stamp requests canceling.");
                return;
            }
            Builder b = prepareBuffer();
    ...
    public synchronized void reqContractDetails(int reqId, Contract contract)
    {
        ...
        if (m_serverVersion < MIN_SERVER_VER_LINKING)
        {
            ...
            {
                error(reqId, EClientErrors.UPDATE_TWS, "  It does not support primaryExchange parameter in reqContractDetails.");
                return;
            }
        }
        final int VERSION = 8;
        try
        {
            ...
            {
                if (!IsEmpty(contract.primaryExch()) && ("BEST".equals(contract.exchange()) || "SMART".equals(contract.exchange())))
                {
    ...
    public synchronized void reqMktDepth(int tickerId, Contract contract, int numRows, boolean isSmartDepth, List<TagValue> mktDepthOptions)
    {
        ...
        {
            error(EClientErrors.NO_VALID_ID, EClientErrors.UPDATE_TWS, "  It does not support SMART depth request.");
            return;
        }
        if (m_serverVersion < MIN_SERVER_VER_MKT_DEPTH_PRIM_EXCHANGE && !IsEmpty(contract.primaryExch()))
        {
            error(EClientErrors.NO_VALID_ID, EClientErrors.UPDATE_TWS, "  It does not support primaryExch parameter in reqMktDepth.");
            return;
        }
        final int VERSION = 5;
        try
    ...
    public synchronized void cancelMktDepth(int tickerId, boolean isSmartDepth)
    {
        ...
        if (m_serverVersion < MIN_SERVER_VER_SMART_DEPTH && isSmartDepth)
        {
            error(EClientErrors.NO_VALID_ID, EClientErrors.UPDATE_TWS, "  It does not support SMART depth cancel.");
            return;
        }
        final int VERSION = 1;
        // send cancel mkt data msg
    ...
    public synchronized void placeOrder(int id, Contract contract, Order order)
    {
        ...
        {
            if (order.deltaNeutralConId() > 0 || !IsEmpty(order.deltaNeutralSettlingFirm()) || !IsEmpty(order.deltaNeutralClearingAccount())
        ...
        {
            if (!IsEmpty(order.deltaNeutralOpenClose()) || order.deltaNeutralShortSale() || order.deltaNeutralShortSaleSlot() > 0
        ...
        if (m_serverVersion < MIN_SERVER_VER_SCALE_ORDERS3)
        {
            ...
            {
                if (order.scalePriceAdjustValue() != Double.MAX_VALUE || order.scalePriceAdjustInterval() != Integer.MAX_VALUE
                        || order.scaleProfitOffset() != Double.MAX_VALUE || order.scaleAutoReset() || order.scaleInitPosition() != Integer.MAX_VALUE
        ...
        if (m_serverVersion < MIN_SERVER_VER_SOFT_DOLLAR_TIER && (!IsEmpty(order.softDollarTier().name()) || !IsEmpty(order.softDollarTier().value())))
        {
        ...
        if (m_serverVersion < MIN_SERVER_VER_CASH_QTY)
        {
            ...
            {
                error(id, EClientErrors.UPDATE_TWS, " It does not support cash quantity parameter");
                return;
            }
        }
        if (m_serverVersion < MIN_SERVER_VER_DECISION_MAKER && (!IsEmpty(order.mifid2DecisionMaker()) || !IsEmpty(order.mifid2DecisionAlgo())))
        {
            error(id, EClientErrors.UPDATE_TWS, " It does not support MIFID II decision maker parameters");
            return;
        }
        if (m_serverVersion < MIN_SERVER_VER_MIFID_EXECUTION && (!IsEmpty(order.mifid2ExecutionTrader()) || !IsEmpty(order.mifid2ExecutionAlgo())))
        {
            error(id, EClientErrors.UPDATE_TWS, " It does not support MIFID II execution parameters");
            return;
        }
        if (m_serverVersion < MIN_SERVER_VER_AUTO_PRICE_FOR_HEDGE && order.dontUseAutoPriceForHedge())
        {
            error(id, EClientErrors.UPDATE_TWS, "  It does not support don't use auto price for hedge parameter.");
            return;
        }
        if (m_serverVersion < MIN_SERVER_VER_ORDER_CONTAINER && order.isOmsContainer())
        {
            error(id, EClientErrors.UPDATE_TWS, "  It does not support oms container parameter.");
            return;
        }
        if (m_serverVersion < MIN_SERVER_VER_D_PEG_ORDERS && order.discretionaryUpToLimitPrice())
        {
            error(id, EClientErrors.UPDATE_TWS, "  It does not support D-Peg orders.");
            return;
        }
        if (m_serverVersion < MIN_SERVER_VER_PRICE_MGMT_ALGO && order.usePriceMgmtAlgo() != null)
        {
        ...
        try
        {
            ...
            if (m_serverVersion >= MIN_SERVER_VER_FRACTIONAL_POSITIONS) b.send(order.totalQuantity());
            else b.send((int) order.totalQuantity());
            ...
            if (m_serverVersion >= 19)
            {
                ...
                double lower = (m_serverVersion == 26 && order.getOrderType().equals("VOL")) ? Double.MAX_VALUE : order.stockRangeLower();
                double upper = (m_serverVersion == 26 && order.getOrderType().equals("VOL")) ? Double.MAX_VALUE : order.stockRangeUpper();
                b.sendMax(lower);
                b.sendMax(upper);
            }
    ...
    public synchronized void reqSecDefOptParams(int reqId, String underlyingSymbol, String futFopExchange, String underlyingSecType, int underlyingConId)
    {
        ...
        {
            error(EClientErrors.NO_VALID_ID, EClientErrors.UPDATE_TWS, "  It does not support security definition option requests.");
            return;
        }
        Builder b = prepareBuffer();
    ...
    public synchronized void reqSoftDollarTiers(int reqId)
    {
        ...
        if (m_serverVersion < MIN_SERVER_VER_SOFT_DOLLAR_TIER)
        {
            error(EClientErrors.NO_VALID_ID, EClientErrors.UPDATE_TWS, "  It does not support soft dollar tier requests.");
            return;
        }
        Builder b = prepareBuffer();
    ...
    public synchronized void reqPositionsMulti(int reqId, String account, String modelCode)
    {
        ...
        if (m_serverVersion < MIN_SERVER_VER_MODELS_SUPPORT)
        {
            error(EClientErrors.NO_VALID_ID, EClientErrors.UPDATE_TWS, "  It does not support positions multi request.");
            return;
        }
        final int VERSION = 1;
        Builder b = prepareBuffer();
    ...
    public synchronized void cancelPositionsMulti(int reqId)
    {
        ...
        if (m_serverVersion < MIN_SERVER_VER_MODELS_SUPPORT)
        {
            error(EClientErrors.NO_VALID_ID, EClientErrors.UPDATE_TWS, "  It does not support positions multi cancellation.");
            return;
        }
        final int VERSION = 1;
        Builder b = prepareBuffer();
    ...
    public synchronized void cancelAccountUpdatesMulti(int reqId)
    {
        ...
        if (m_serverVersion < MIN_SERVER_VER_MODELS_SUPPORT)
        {
            error(EClientErrors.NO_VALID_ID, EClientErrors.UPDATE_TWS, "  It does not support account updates multi cancellation.");
            return;
        }
        final int VERSION = 1;
        Builder b = prepareBuffer();
    ...
    public synchronized void reqAccountUpdatesMulti(int reqId, String account, String modelCode, boolean ledgerAndNLV)
    {
        ...
        if (m_serverVersion < MIN_SERVER_VER_MODELS_SUPPORT)
        {
            error(EClientErrors.NO_VALID_ID, EClientErrors.UPDATE_TWS, "  It does not support account updates multi requests.");
            return;
        }
        final int VERSION = 1;
        Builder b = prepareBuffer();
    ...
    public synchronized void reqMatchingSymbols(int reqId, String pattern)
    {
        ...
        {
            error(EClientErrors.NO_VALID_ID, EClientErrors.UPDATE_TWS, "  It does not support matching symbols request.");
            return;
        }
        Builder b = prepareBuffer();
    ...
    public synchronized void reqFamilyCodes()
    {
        ...
        {
            error(EClientErrors.NO_VALID_ID, EClientErrors.UPDATE_TWS, "  It does not support family codes request.");
            return;
        }
        Builder b = prepareBuffer();
    ...
    public synchronized void reqMktDepthExchanges()
    {
        ...
        {
            error(EClientErrors.NO_VALID_ID, EClientErrors.UPDATE_TWS, "  It does not support market depth exchanges request.");
            return;
        }
        Builder b = prepareBuffer();
    ...
    public synchronized void reqSmartComponents(int reqId, String bboExchange)
    {
        ...
        if (m_serverVersion < MIN_SERVER_VER_REQ_SMART_COMPONENTS)
        {
            error(EClientErrors.NO_VALID_ID, EClientErrors.UPDATE_TWS, "  It does not support smart components request.");
            return;
        }
        Builder b = prepareBuffer();
    ...
    public synchronized void reqNewsProviders()
    {
        ...
        {
            error(EClientErrors.NO_VALID_ID, EClientErrors.UPDATE_TWS, "  It does not support news providers request.");
            return;
        }
        Builder b = prepareBuffer();
    ...
    public synchronized void reqNewsArticle(int requestId, String providerCode, String articleId, List<TagValue> newsArticleOptions)
    {
        ...
        {
            error(EClientErrors.NO_VALID_ID, EClientErrors.UPDATE_TWS, "  It does not support news article request.");
            return;
        }
        Builder b = prepareBuffer();
    ...
    public synchronized void reqHistoricalNews(int requestId, int conId, String providerCodes, String startDateTime, String endDateTime, int totalResults,
            List<TagValue> historicalNewsOptions)
    {
        ...
        {
            error(EClientErrors.NO_VALID_ID, EClientErrors.UPDATE_TWS, "  It does not support historical news request.");
            return;
        }
        Builder b = prepareBuffer();
    ...
    public synchronized void reqHistogramData(int tickerId, Contract contract, boolean useRTH, String timePeriod)
    {
        ...
        try
        {
            ...
            {
                error(tickerId, EClientErrors.UPDATE_TWS, "  It does not support histogram requests.");
                return;
            }
            Builder b = prepareBuffer();
    ...
    public synchronized void reqMarketRule(int marketRuleId)
    {
        ...
        {
            error(EClientErrors.NO_VALID_ID, EClientErrors.UPDATE_TWS, "  It does not support market rule requests.");
            return;
        }
    ...
    public synchronized void reqPnL(int reqId, String account, String modelCode)
    {
        ...
        {
            error(reqId, EClientErrors.UPDATE_TWS, "  It does not support PnL requests.");
            return;
        }
        try
        {
    ...
    public synchronized void reqHistoricalTicks(int reqId, Contract contract, String startDateTime, String endDateTime, int numberOfTicks, String whatToShow, int useRth,
            boolean ignoreSize, List<TagValue> miscOptions)
    {
        ...
        {
            error(reqId, EClientErrors.UPDATE_TWS, "  It does not support historical ticks request.");
            return;
        }
        try
        {
    ...
    public synchronized void reqTickByTickData(int reqId, Contract contract, String tickType, int numberOfTicks, boolean ignoreSize)
    {
        ...
        {
            error(EClientErrors.NO_VALID_ID, EClientErrors.UPDATE_TWS, "  It does not support tick-by-tick data requests.");
            return;
        }
        if (m_serverVersion < MIN_SERVER_VER_TICK_BY_TICK_IGNORE_SIZE)
        {
            ...
            {
                error(EClientErrors.NO_VALID_ID, EClientErrors.UPDATE_TWS, "  It does not support ignoreSize and numberOfTicks parameters in tick-by-tick data requests.");
                return;
            }
        }
        try
        ...
        {
            error(EClientErrors.NO_VALID_ID, EClientErrors.FAIL_SEND_REQTICKBYTICK, e.toString());
            close();
        }
    }
    ...
    public synchronized void cancelTickByTickData(int reqId)
    {
        ...
        if (m_serverVersion < MIN_SERVER_VER_TICK_BY_TICK)
        {
            error(EClientErrors.NO_VALID_ID, EClientErrors.UPDATE_TWS, "  It does not support tick-by-tick data cancels.");
            return;
        }
        try
        {
        ...
        {
            error(EClientErrors.NO_VALID_ID, EClientErrors.FAIL_SEND_CANTICKBYTICK, e.toString());
            close();
        }
    }
    ...
    public synchronized void reqCompletedOrders(boolean apiOnly)
    {
        ...
        {
            error(EClientErrors.NO_VALID_ID, EClientErrors.UPDATE_TWS, "  It does not support completed orders requests.");
            return;
        }
        try
        {
        ...
        {
            error(EClientErrors.NO_VALID_ID, EClientErrors.FAIL_SEND_REQ_COMPLETED_ORDERS, e.toString());
            close();
        }
    }
